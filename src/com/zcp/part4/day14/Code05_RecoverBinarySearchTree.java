...
public class Code05_RecoverBinarySearchTree {
    ...
    public void recoverTree3(TreeNode root) {
        ...
        for (int i = 1; i < list.size(); i++) {
            if (firstErrorNode == null && list.get(i - 1).val > list.get(i).val) {
                firstErrorNode = list.get(i - 1);
            }
            if (list.get(i - 1).val > list.get(i).val) {
                lastErrorNode = list.get(i);
            }
        }

        int val = firstErrorNode.val;
        firstErrorNode.val = lastErrorNode.val;
        lastErrorNode.val = val;
    }

    public void process3(TreeNode root, List<TreeNode> list) {
        if (root == null) {
            return;
        }
        process3(root.left, list);
        list.add(root);
        process3(root.right, list);
    }
