...
public class Code05_CardsProblem {
     ...
     * @return
     */
    public static int ways3(String[] cards) {
        if (cards == null || cards.length < 1) {
            return 0;
        }
        int totalCount = 0;
        int[] fullArray = new int[27];
        for (int i = 0; i < cards.length; i++) {
            fullArray[calcIndex(cards[i])]++;
        }
        //统计相同牌面数
        for (int i = 0; i < fullArray.length; i++) {
            ...
            if (n >= 3) {
                //求n的组合数
                totalCount += (n * (n - 1) * (n - 2)) / (1 * 2 * 3);
            }
        }

        //不同牌面组合数
        totalCount += process3(fullArray, 0, new ArrayList<>());

        return totalCount;
    }

    /**
     * 当前在i位置，任意选择3张，selected表示之前选择的卡片，返回满足条件的组合数
     *
     * @param fullArray
     * @param i
     * @param selected
     * @return
     */
    private static int process3(int[] fullArray, int i, ArrayList<String> selected) {
        if (selected.size() == 3) {
            //判断是否满足条件并且返回组合数
            return getNumberOfCombinations(fullArray, selected);
        }
        ...
        //p1选择
        //p2不选择
        String card = getStringByIndex(i);
        selected.add(card);
        int p1 = process3(fullArray, i + 1, selected);
        selected.remove(selected.size() - 1);
        int p2 = process3(fullArray, i + 1, selected);
        return p1 + p2;
    }
     ...
     * @param selected
     */
    private static int getNumberOfCombinations(int[] fullArray, ArrayList<String> selected) {
        String card1 = selected.get(0);
        String card2 = selected.get(1);
        String card3 = selected.get(2);

        for (int i = 0; i < 3; i++) {
            char c1 = card1.charAt(i);
            char c2 = card2.charAt(i);
            char c3 = card3.charAt(i);
            if ((c1 == c2 && c2 != c3) || (c1 == c3 && c2 != c3) || (c2 == c3 && c1 != c2)) {
                return 0;
            }
        }
        //说明组合有效计算组合数
        return fullArray[calcIndex(card1)] * fullArray[calcIndex(card2)] * fullArray[calcIndex(card3)];
    }

    /**
     * 根据string计算下标
     * 如：“ABC” = 9*0+3*1+1*2 = 5, "CCC" = 9*2+3*2+1*2 = 26
     *
     * @param card
     * @return
     */
    private static int calcIndex(String card) {
        int index = 0;
        int k = 1;
        for (int i = 0; i < 3; i++) {
            index += (card.charAt(i) - 'A') * k;
            k *= 3;
        }
        return index;
    }
