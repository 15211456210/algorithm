...
public class Code07_TargetSum {
    ...
    public int findTargetSumWays4(int[] nums, int target) {
        ...
        //dp[i][j]  从i位置开始，后面的数随便选择符号，累计和为 (sum+j)的组合数
        //，因为rest可能是负数， int[i][j] j=sum 代表0
        int M = 2 * sum + 1;
        int[][] dp = new int[N + 1][M];
        dp[N][sum] = 1;
        for (int i = N - 1; i >= 0; i--) {
            for (int j = 0; j < M; j++) {
                int p1 = j + ints[i] >= M ? 0 : dp[i + 1][j + ints[i]];
                int p2 = j - ints[i] < 0 ? 0 : dp[i + 1][j - ints[i]];
                dp[i][j] = p1 + p2;
            }
        }
    ...
    public int findTargetSumWays3(int[] nums, int target) {
        ...
        return process3(ints, 0, target);
    }

    /**
     * 下标从index位置开始，后面符号自由选择，累计和为rest的方法数
     *
     * @param nums
     * @param index
     * @param rest
     * @return
     */
    public int process3(int[] nums, int index, int rest) {
        if (index == nums.length) {
            return rest == 0 ? 1 : 0;
        }
        //还有数
        //两种选择
        return process3(nums, index + 1, rest + nums[index]) + process3(nums, index + 1, rest - nums[index]);
    }
