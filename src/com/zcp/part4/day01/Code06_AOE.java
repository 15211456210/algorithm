...
public class Code06_AOE {
     ...
     * 思路：
     * 贪心
     * 从最左大于0的怪开始释放AOE，并且每次释放AOE的左边界都是最左边血量不为0的怪
     *
     * @param x
     * @param hp
     * @param range
    ...
    public static int minAoe4(int[] x, int[] hp, int range) {
        if (x == null || hp == null || range <= 0 || x.length != hp.length) {
            return -1;
        }
        int L = 0;
        int conut = 0;
        while (L < x.length) {
            if (hp[L] > 0) {
                ...
                int curIndex = x[L];
                int cL = L;
                while (cL < x.length && x[cL] < curIndex + range * 2 + 1) {
                    hp[cL] -= cut;
                    cL++;
                }
            }
