...
public class Code03_FindWordInMatrix {
     ...
     * @return
     */
    public static boolean findWord4(char[][] m, String word) {
        if (word.length() == 0) {
            return true;
        }
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                ans |= process4(m, i, j, 0, word.toCharArray());
            }
        }
        return ans;
    ...
    /**
     * 从m[i][j]位置开始 能否走出word[...k...]k往后的字符串
     *
     * @param m
     * @param i
     * @param j
     ...
     * @return
     */
    public static boolean process4(char[][] m, int i, int j, int k, char[] word) {
        if (k == word.length) {
            return true;
        }
        if (i < 0 || i >= m.length || j < 0 || j >= m[0].length || m[i][j] != word[k] || m[i][j] == 0) {
            return false;
        }
        //word[k] = m[i][j]
        //将当前位置设置成ascii码设置成0：表示当前位置已经走过了
        m[i][j] = 0;
        boolean ans = false;
        ans |= process3(m, i - 1, j, k + 1, word);
        ans |= process3(m, i + 1, j, k + 1, word);
        ans |= process3(m, i, j - 1, k + 1, word);
        ans |= process3(m, i, j + 1, k + 1, word);
        //在返回结果之前 把当前位置0 改回原来的值
        m[i][j] = word[k];
        return ans;
    }
     ...
     * @return
     */
    public static boolean findWord3(char[][] m, String word) {
        if (word.length() == 0) {
            return true;
        }
        if (m == null || m.length < 1) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                ans |= process3(m, i, j, 0, word.toCharArray());
            }
        }
        return ans;
    }
     ...
     * @return
     */
    public static boolean process3(char[][] m, int i, int j, int k, char[] word) {
        if (k == word.length) {
            return true;
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(findWord1(m, word3));
        System.out.println("----");
        System.out.println(findWord3(m, word1));
        System.out.println(findWord3(m, word2));
        System.out.println(findWord3(m, word3));
        System.out.println("----");
        // 不可以走重复路的设定
        System.out.println(findWord2(m, word1));
        System.out.println(findWord2(m, word2));
        System.out.println(findWord2(m, word3));
        System.out.println("----");
        System.out.println(findWord4(m, word1));
        System.out.println(findWord4(m, word2));
        System.out.println(findWord4(m, word3));
