...
public class Code02_LargestComponentSizebyCommonFactor {
    ...
    public int largestComponentSize(int[] nums) {
        ...
        for (int i = 0; i < len; i++) {
            updateIndexMap(i, nums[i], indexMap, unionFind);
        }
    ...
    /**
     * 将num所有因子添加到indexMap,并且需要更新unionfind
     *
     * @param index
     * @param num
     * @param indexMap
     * @param unionFind
     */
    private void updateIndexMap(int index, int num, HashMap<Integer, Integer> indexMap, UnionFind unionFind) {
        for (int i = 2; i <= Math.sqrt(num); i++) {
            if (num % i == 0) {
                int another = num / i;
                if (indexMap.containsKey(i)) {
                    unionFind.union(index, indexMap.get(i));
                } else {
                    indexMap.put(i, index);
                }
                if (indexMap.containsKey(another)) {
                    unionFind.union(index, indexMap.get(another));
                } else {
                    indexMap.put(another, index);
                }
            }
        }
        if (indexMap.containsKey(num)) {
            unionFind.union(index, indexMap.get(num));
        } else {
