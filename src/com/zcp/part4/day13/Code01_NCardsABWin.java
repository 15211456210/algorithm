...
public class Code01_NCardsABWin {
    ...
    // 当累加和>=b时，你将失败
    // 返回获胜的概率，给定的参数为N，a，b
    public static double f5(int N, int low, int up) {
        if (N <= 0 || low < 0 || up < 0 || low >= up) {
            return 0;
        }
        return process5(N, 0, low, up);
    }
     ...
     * @return
     */
    public static double process5(int N, int cNum, int low, int up) {
        if (cNum >= low && cNum < up) {
            return 1;
        }
        if (cNum >= up) {
        ...
        double total = 0;
        for (int i = 1; i <= N; i++) {
            total += process5(N, cNum + i, low, up);
        }
        return total / N;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTime; i++) {
            ...
            double ans5 = new BigDecimal(f5(N, a, b)).setScale(4, BigDecimal.ROUND_HALF_UP).doubleValue();
            if (ans2 != ans3 || ans2 != ans4 || ans2 != ans5) {
