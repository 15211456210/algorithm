...
public class Code05_BestTimeToBuyAndSellStockWithCooldown {
    ...
    public int maxProfit4(int[] prices) {
        ...
        return process2(prices, 0, false);
    }

    /**
     * 从第index天开始买股票，最大收益是多少
     *
     * @param prices
     * @param index
     * @param hold   是否持有股
     * @return
     */
    public int process2(int[] prices, int index, boolean hold) {
        ...
        if (hold) {
            //p1:不卖持有股
            int p1 = process2(prices, index + 1, true);
            //p2: 卖出持有股
            int p2 = prices[index] + process2(prices, index + 2, false);
            maxBenifit = Math.max(p1, p2);
        } else {
            //p1:不买当前股
            int p1 = process2(prices, index + 1, false);
            //p2：买当前股
            int p2 = -prices[index] + process2(prices, index + 1, true);
            maxBenifit = Math.max(p1, p2);
        }
        return maxBenifit;
    }
