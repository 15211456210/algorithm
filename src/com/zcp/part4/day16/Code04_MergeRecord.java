...
public class Code04_MergeRecord {
     ...
     * @return
     */
    public static int[] reversePair3(int[] originArr, int[] reverseArr, int power) {
        if (originArr == null || reverseArr == null || originArr.length != Math.pow(2, power)) {
            return null;
        }
        //获取逆序数组
        int[] reverse = reverse(originArr);

        //reverseOrderArray[i]: 以2^i次方为长度分组，对应的逆序对数量；
        //positivePairArray[i]: 以2^i次方为长度分组，对应的正序对数量；
        int[] reverseOrderArray = new int[power + 1];
        int[] positivePairArray = new int[power + 1];

        merge3(originArr, reverseOrderArray, power, 0, originArr.length - 1);
        merge3(reverse, positivePairArray, power, 0, originArr.length - 1);
        int len = reverseArr.length;
        int[] ans = new int[len];
        for (int i = 0; i < len; i++) {
            //计算结果
            //如果reverseArr[i] = m ，那么所有m幂次以下的逆序对和正续对都需要数值对调
            for (int j = 0; j <= reverseArr[i]; j++) {
                int tmp = reverseOrderArray[j];
                reverseOrderArray[j] = positivePairArray[j];
                positivePairArray[j] = tmp;
            }
            //最终总的逆序对为 sum(quota[i][0])
            for (int m = 0; m <= power; m++) {
                ans[i] += reverseOrderArray[m];
            }
        }
        return ans;
    }
     ...
     * @return
     */
    private static int[] reverse(int[] originArr) {
        int[] arr = new int[originArr.length];
     ...
     * @param originArr
     * @param quota
     * @param power     当前分段处在2的几次幂
     * @param l
     * @param r
     */
    public static void merge3(int[] originArr, int[] quota, int power, int l, int r) {
        if (l == r || power == 0) {
            //一个数一组无需计算
            return;
        }
        ...
        int mid = l + (r - l) / 2;
        //先递归，让左右子数组有序
        merge3(originArr, quota, power - 1, l, mid);
        merge3(originArr, quota, power - 1, mid + 1, r);
//        int segmentLen = r - l + 1;
        //[l...mid]  [mid+1...r]
        //归并排序 + 收集quota数据
        int pl = l;
        int pr = mid + 1;
        while (pl <= mid && pr <= r) {
            if (originArr[pl] > originArr[pr]) {
                quota[power] += mid - pl + 1;
                tmp[index++] = originArr[pr++];
            } else {
                tmp[index++] = originArr[pl++];
            }
        }
        while (pl <= mid) {
            tmp[index++] = originArr[pl++];
...
//            System.out.println("reverseArr:" + Arrays.toString(reverseArr));
            ...
            int[] ans3 = reversePair3(originArr3, reverseArr3, power);
        ...
        reversePair3(originArr, reverseArr, powerMax);
