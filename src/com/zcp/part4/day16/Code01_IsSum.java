...
public class Code01_IsSum {
     ...
     * @return
     */
    public static boolean isSum5(int[] arr, int sum) {

        if (arr == null || arr.length < 1) {
            return sum == 0;
        }
        List<Integer> list = new ArrayList<>();
        process5(arr, 0, 0, list);
     ...
     * @param list
     */
    public static void process5(int[] arr, int i, int sum, List<Integer> list) {
        if (i == arr.length) {
            list.add(sum);
            return;
        }
        process5(arr, i + 1, sum, list);
        process5(arr, i + 1, sum + arr[i], list);
